...
def format_image_sketch(img_path, size, nb_channels):
    """
    Load img with opencv and reshape
    """
...
def build_HDF5(jpeg_dir, nb_channels, data_dir, size=256):
    ...
    with h5py.File(hdf5_file, "w") as hfw:
        ...
        for dset_type in ["train", "test"]:#, "val"]:
            ...
            list_img = [img for img in Path(jpeg_dir).glob('DEPTH/%s/*.jpg' % dset_type)]
            list_img = [str(img) for img in list_img]
            list_img.extend(list(Path(jpeg_dir).glob('DEPTH/%s/*.png' % dset_type)))
            list_img = list(map(str, list_img))
            list_img = np.array(list_img)
            
            list_img_sketch = [img for img in Path(jpeg_dir).glob('RGB/%s/*.jpg' % dset_type)]
            list_img_sketch = [str(img) for img in list_img_sketch]
            list_img_sketch.extend(list(Path(jpeg_dir).glob('RGB/%s/*.png' % dset_type)))
            list_img_sketch = list(map(str, list_img_sketch))
            list_img_sketch = np.array(list_img_sketch)
            ...
            for chunk_idx in tqdm(arr_chunks):
                ...
                list_img_sketch_path = list_img_sketch[chunk_idx].tolist()
                output_full = parmap.map(format_image, list_img_path, size, nb_channels, pm_parallel=False)
                output_sketch = parmap.map(format_image_sketch, list_img_sketch_path, size, 3, pm_parallel=False)# nb_channels= 3 for RGB


                arr_img_full = np.concatenate([o for o in output_full], axis=0)
                arr_img_sketch = np.concatenate([o for o in output_sketch], axis=0)
...
def check_HDF5(jpeg_dir,data_dir, nb_channels):
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument('jpeg_dir', default='D:/CurtinFaces_crop_gan/data/CurtinFaces', type=str, help='path to jpeg images')
    parser.add_argument('nb_channels', default=3, type=int, help='number of image channels')
    parser.add_argument('--img_size', default=256, type=int,
                        help='Desired Width == Height')
    parser.add_argument('--do_plot', default=True, action="store_true",
    ...
    if do_plot:
        check_HDF5(jpeg_dir,data_dir, nb_channels)
