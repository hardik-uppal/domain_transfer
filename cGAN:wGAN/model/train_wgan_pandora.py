...
#    generator_model.compile(loss='mae', optimizer=opt_G)
    ...
    generator_model = models_WGAN.generator_upsampling(noise_dim, img_dim, n_class, model_name="generator_upsampling")
...
#    loss_weights = [0.5, 0.5]
    generator_model.compile(loss=['mae','categorical_crossentropy'], optimizer=opt_G, metrics=['mse'])
#    loss_weights_gan = [1, 1E1]
    discriminator_model.trainable = False
    DCGAN_model.compile(loss=[models_WGAN.wasserstein, 'mae', 'categorical_crossentropy'], optimizer=opt_G, metrics=['mse'])
...
#    generator_both_train = data_utils.flow_from_both_dir(rgb_train_dir, batch_size, img_dim,'pandora','train')
    generator_both_test = data_utils.flow_from_both_dir_disc(rgb_train_dir, batch_size, img_dim,'pandora','test')
    generator_both_train_disc = data_utils.flow_from_both_dir_disc(rgb_train_dir, batch_size, img_dim,'pandora','train')
...
#                depth_val_dir = 'D:/CurtinFaces_crop/DEPTH/test1/'                
        ...
        for X_rgb_batch, X_depth_batch, y_label in generator_both_train:
            #test stuff
...
#            progbar_disc = generic_utils.Progbar(disc_iterations)
            ...
            for disc_it in range(disc_iterations):
                ...
                X_rgb_batch1, X_depth_batch1, y_label1 = next(generator_both_train_disc)
...
#            X_gen_noise = data_utils.sample_noise(noise_scale, batch_size, noise_dim)
        # Freeze the discriminator
            discriminator_model.trainable = False
            gen_loss = DCGAN_model.train_on_batch([X_rgb_batch], [-np.ones(X_rgb_batch.shape[0]), X_depth_batch, y_label])
...
#                                                ("Loss_ae_cat", np.mean(list_ae_loss,axis=0)[1]),
                                            ("WGAN tot", gen_loss[0]),
                                            ("WGAN disc loss", -gen_loss[1]),
                                            ("WGAN mae", gen_loss[2]),
                                            ("WGAN cat loss", gen_loss[3]),
                                            ("WGAN (5)mse", gen_loss[5]),
                                            ("WGAN 4", gen_loss[4]),
                                            ("WGAN 6", gen_loss[6])])
...
#                        fd = fid.FrechetInceptionDistance(generator_model, (0,1)) 
                ...
                if e%10 == 0:
                    X_rgb_batch, X_depth_batch, y_label = next(generator_both_test)
...
#                        noise_input = data_utils.sample_noise(noise_scale, batch_size, noise_dim)
                    x_batch_gen_depth, pred_class = generator_model.predict([X_rgb_batch])

                    metric_mae = K.eval(mean_absolute_error(K.variable(X_depth_batch), K.variable(x_batch_gen_depth)))
                    metric_mse = K.eval(mean_squared_error(K.variable(X_depth_batch), K.variable(x_batch_gen_depth)))
                    metric_sae = K.eval(sum_absolute_error(K.variable(X_depth_batch), K.variable(x_batch_gen_depth)))
                    metric_sse = K.eval(sum_squared_error(K.variable(X_depth_batch), K.variable(x_batch_gen_depth)))                    
                    metric_rmse = K.eval(rmse(K.variable(X_depth_batch), K.variable(x_batch_gen_depth)))
                    print('\n')
                    print('\n MAE: {}, MSE: {}, RMSE: {}, sAE: {}, sSE: {},'.format(metric_mae, metric_mse, metric_rmse,metric_sae,metric_sse))
#                        try:
