...
def Caps_att(input_shape,input_shape_depth, n_class):
    """
...
##    inputs_depth = Lambda(lambda x: x, name = "inputs_depth")(input_depth_val)
    inputs_depth = layers.Input(shape=input_shape_depth, name = "inputs_depth")
...
#
    ...
    # removed for capsule
    fc6 = layers.Dense(1024, activation='relu', name='fc6')(flat_model_rgb)
...
#
#
    fc7 = layers.Dense(1024, activation='relu', name='fc7')(flat_model_depth)
    bn_2 = BatchNormalization(name='2_bn')(fc7)
#    dropout_2 = layers.Dropout(0.5)(bn_2)
    
    fc8 = layers.Dense(2048, activation='relu', name='fc8')(flat_model)
...
#    dropout_3 = layers.Dropout(0.5)(bn_3)
    ...
    train_model = models.Model(inputs=[inputs_rgb, inputs_depth], outputs=[output,output_rgb,output_depth])
...
def train(model, generate_depth, args):
    """
    Training 
...
#    (x_train, y_train), (x_test, y_test) = data
    ...
    es_cb = callbacks.EarlyStopping(monitor='val_output_acc', patience=10, verbose=1, mode='auto')
#    tb = callbacks.TensorBoard(log_dir=args.save_dir + '/tensorboard-logs',
#                               batch_size=args.batch_size, histogram_freq=int(args.debug))
    checkpoint = callbacks.ModelCheckpoint(args.save_dir + '/weights-best.h5', monitor='val_output_acc',
...
#        depth_train_dir = 'D:/CurtinFaces_crop/RGB/test1'   #D:/CurtinFaces_crop/normalized/DEPTH/train/
...
#        depth_val_dir = 'D:/VAP_trimodal/depth/test/'     #D:/CurtinFaces_crop/normalized/DEPTH/test1/
        ...
        generator_rgb = train_datagen.flow_from_directory(directory= rgb_train_dir, target_size=(128, 128), color_mode="rgb",
...
#                                                      batch_size=batch_size, class_mode="categorical", shuffle=True,subset='training', seed=42)
        ...
        generator_rgb_val = train_datagen.flow_from_directory(directory= rgb_train_dir, target_size=(128, 128), color_mode="rgb",
...
#                                                      batch_size=batch_size, class_mode="categorical", shuffle=True,subset='validation', seed=42)
        ...
        generator_model = model_lib.load("generator_unet%s" % generate_depth,
                                      img_dim,
                                      nb_patch,
                                      bn_mode,
                                      use_mbd,
                                      batch_size,
                                      do_plot,n_class=48)
        opt_discriminator = optimizers.Adam(lr=1E-3, beta_1=0.9, beta_2=0.999, epsilon=1e-08)
...
#            print('train\n')
            while 1:
                ...
                ## depth data aug
                x_batch_depth,pred_class = generator_model.predict(x_batch_rgb)
                ...
                yield [[x_batch_rgb_final, x_batch_depth_final], [y_batch_rgb_final,y_batch_rgb_final,y_batch_depth_final]]
        elif val_train == 'val':
            while 1:
...
#            
    ...
    model.fit_generator(generator=train_generator(args.batch_size,'train'),
                        steps_per_epoch=int(884 / int(args.batch_size/5)),##936 curtin faces###424 fold1 iiitd ##46846
                        epochs=args.epochs,
                        validation_data=train_generator(args.batch_size,'val'),
                        validation_steps = int(52 / int(args.batch_size)),##4108 curtin faces###4181 fold1 iiitd ##39942
                        callbacks=[log, checkpoint, lr_decay, es_cb])
...
def test(model, generate_depth ,args):
...
#        while 1:
    ...
    end_test1 = time.time()
    print('Test loss: {} ; Accuracy on Test: {} in time {}'.format(scores[0:4],scores[4:], (end_test1 - start_test1)))
    import csv
    test_log = args.save_dir + '/log_test3.csv'
    with open(test_log, 'w', newline='') as csvfile:
        spamwriter = csv.writer(csvfile)
        spamwriter.writerow(['Test loss: {} ; Accuracy on Test: {} in time {}'.format(scores[0:4],scores[4:], (start_test1 - end_test1)/2028)])
    ...
    end_test2 = time.time()
    print('Test loss: {} ; Accuracy on Test: {} in time {}'.format(scores[0:4],scores[4:], (end_test2 - start_test2)))
    import csv
    test_log = args.save_dir + '/log_test2.csv'
    with open(test_log, 'w', newline='') as csvfile:
        spamwriter = csv.writer(csvfile)
        spamwriter.writerow(['Test loss: {} ; Accuracy on Test: {} in time {}'.format(scores[0:4],scores[4:], (start_test2 - end_test2)/1560)])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_dir', default='./Face_id_results_gen_depth/unet_mae')
...
#    weight_path = 'D:/tutorial/hulk_dump/results/unet_mae/gen_weights_epoch200.h5'
...
#    print('generator weights loaded....')
    # define model
    model = Caps_att(input_shape=img_dim,input_shape_depth=img_dim_depth, n_class=52)
    model.summary()
#    model.load_weights('./TFIS/CurtinFaces_CAM_final_lr4_bacth1/weights-best.h5')

#    start_train =time.time()
    model = train(model=model, generate_depth=generator ,args=args)
#    end_train =time.time()
...
##                  loss_weights={'output':0.5, 'output_rgb': 0.25,'output_depth':0.25},
#                  metrics=['accuracy'])
    test(model=model, generate_depth=generator ,args=args)
