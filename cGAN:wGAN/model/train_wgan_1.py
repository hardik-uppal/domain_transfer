...
#    loss_weights = [0.5, 0.5]
    ...
    loss_weights_gan = [0.5, 0.5, 1E1]
    discriminator_model.trainable = False
    DCGAN_model.compile(loss=[ models_WGAN.wasserstein,'mae','categorical_crossentropy'],loss_weights=loss_weights_gan, optimizer=opt_G, metrics=['accuracy'])
    discriminator_model.trainable = True
    discriminator_model.compile(loss=models_WGAN.wasserstein, optimizer=opt_D)        
    
    ##load weights
    generator_model.load_weights('D:/tutorial/hulk_dump/gen_weights_epoch90.h5')
...
#            for X_sketch_batch, X_full_batch, y_label in generator_both:
        ...
        # Freeze the discriminator
            discriminator_model.trainable = False
            gen_loss = DCGAN_model.train_on_batch([X_gen_noise,X_rgb_batch], [-np.ones(X_rgb_batch.shape[0]),X_depth_batch,y_label])
